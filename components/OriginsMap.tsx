  ...
  return (
    <section className="relative min-h-screen pt-24 flex flex-col bg-background-light overflow-hidden">
      ...
      <div className="relative w-full flex-grow flex flex-col lg:flex-row bg-white overflow-hidden border-t border-black/5">
        <div className="relative w-full lg:w-2/3 xl:w-3/4 min-h-[400px] sm:min-h-[500px] lg:min-h-[600px] bg-[#fcfbf9] overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center p-4 sm:p-10 opacity-30 pointer-events-none">
            {/* India Shape Placeholder */}
            <svg className="w-full h-full max-h-[60vh] sm:max-h-[70vh] text-[#e6e2de] fill-current" viewBox="0 0 400 500">
          ...
          {REGIONS.map((region) => (
            <div
              key={region.id}
              className={`absolute group cursor-pointer transition-all duration-300 ${selectedRegion.id === region.id ? 'scale-110 sm:scale-125' : 'opacity-60 hover:opacity-100'}`}
            ...
            >
              <div className="relative flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8">
                ...
                <div className={`relative w-3 h-3 sm:w-4 sm:h-4 rounded-full shadow-lg border-2 border-white ${selectedRegion.id === region.id ? 'bg-primary' : 'bg-[#897161]'}`}></div>
              </div>
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 hidden sm:group-hover:block z-20">
                <div className="bg-white px-2 py-1 rounded-lg shadow-xl text-[8px] sm:text-[10px] font-bold tracking-widest uppercase whitespace-nowrap">
                  {region.name}
                </div>
              </div>
            </div>
          ...
          <div className="absolute bottom-4 left-4 sm:bottom-8 sm:left-8 z-20">
            <div className="bg-white/90 backdrop-blur-md rounded-2xl p-3 sm:p-4 shadow-sm border border-black/5 max-w-[150px] sm:max-w-[200px]">
              <h4 className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-[#897161] mb-2 sm:mb-3">Map Layers</h4>
              <div className="space-y-1 sm:space-y-2">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 rounded border border-primary bg-primary flex items-center justify-center">
                    <span className="material-symbols-outlined text-white text-[8px] sm:text-[10px]">check</span>
                  </div>
                  <span className="text-[10px] sm:text-xs font-medium text-[#181411]">Active Estates</span>
                </div>
                <div className="flex items-center gap-2 opacity-50">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 rounded border border-gray-300"></div>
                  <span className="text-[10px] sm:text-xs font-medium text-[#181411]">Historical Routes</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="relative w-full lg:w-1/3 xl:w-1/4 bg-white border-t lg:border-t-0 lg:border-l border-black/5 overflow-y-auto lg:h-auto animate-fade-in-up">
          <div className="p-6 sm:p-8 flex flex-col h-full">
            ...
            <div className="mb-6 sm:mb-8 space-y-4 sm:space-y-6">
              <h3 className="text-3xl sm:text-4xl font-bold text-[#181411] tracking-tight">{selectedRegion.name}</h3>
              <div className="relative w-full aspect-[16/9] lg:aspect-[4/3] rounded-2xl overflow-hidden group">
                <img
            ...
            <div className="grid grid-cols-2 gap-4 mb-8 sm:mb-10">
              <div className="p-4 rounded-xl bg-background-light border border-black/5">
                ...
                <span className="block text-base sm:text-lg font-bold text-[#181411]">{selectedRegion.elevation}</span>
              </div>
              <div className="p-4 rounded-xl bg-background-light border border-black/5">
                <span className="block text-[10px] font-bold text-[#897161] uppercase tracking-widest mb-1">Harvest</span>
                <span className="block text-base sm:text-lg font-bold text-[#181411]">{selectedRegion.harvest}</span>
              </div>
            </div>
